...
export const ZohoCommerceEmbed = ({ embedId, productId, handle, className = "" }: ZohoCommerceEmbedProps) => {
  ...
  useEffect(() => {
    if (scriptLoadedRef.current || !embedRef.current) return;

    const embedElement = embedRef.current;
    
    // Create and inject the Zoho Commerce script
    const script = document.createElement('script');
    script.type = 'text/javascript';
    script.async = true;
    script.src = 'https://ecommerce-stratus.zohostratus.com/IDC/js/zs-buy-button.js';
    
    script.onload = () => {
      // Initialize Zoho Commerce widget
      const config = {
        api_domain: "https://",
        store: "https://",
        type: "product",
      ...
      // @ts-ignore - Zoho's global object
      if (window.zsBuyButton && embedElement) {
        // @ts-ignore
        window.zsBuyButton.init(embedElement, config);
      }
    };
  ...
  }, [embedId, productId, handle]);

  return (
    <div 
      ref={embedRef}
      data-embed-id={embedId}
      className={`zoho-commerce-embed ${className}`}
    />
  );
};