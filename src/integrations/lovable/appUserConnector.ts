/**
 * App User Connector helpers (connection-key flow).
 *
 * SERVER ONLY. Reads server-side secrets from process.env and talks to the
 * Lovable connector gateway. Never import this from a route component, loader,
 * or any file that ends up in the browser bundle.
 *
 * Studio members sign in with the app's own auth (Supabase), so every member
 * gets their own per-user connection key (lovack_*) which we store encrypted
 * server-side and pass back to the gateway on each call.
 */
...
/**
 * A gateway 401 whose body type starts with "credential_" means this member's
 * connection must be re-authorized. No other 401 is a reconnect.
 */
