...
create table public.workspace_settings (
  ...
  preferred_pal text not null default 'kiana' check (preferred_pal in ('kareem', 'kiana', 'ryder', 'raquel', 'cyrus', 'clara', 'silas', 'samira')),
...
create table public.brand_profiles (
  ...
  creator_type text not null default 'Business',
  primary_goal text not null default 'Sell services',
  social_links jsonb not null default '[]'::jsonb,
  brand_details jsonb not null default '{}'::jsonb,
  visual_style text not null default 'Palmer Clay 3D',
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
create table public.content_ideas (
  ...
  source_type text not null default 'text' check (source_type in ('text', 'link', 'image', 'chat', 'recommended')),
  source_url text,
  source_media_path text,
  source_metadata jsonb not null default '{}'::jsonb,
  primary_lane text not null default 'evergreen' check (primary_lane in ('spotlight', 'reel', 'evergreen', 'system')),
  business_problem text not null default '',
  status text not null default 'saved' check (status in ('saved', 'building', 'archived')),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
create table public.workspace_video_items (
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  item_key text not null,
  status text not null default 'recommended' check (status in ('not_needed', 'recommended', 'planned', 'scripted', 'ready_to_film', 'filmed', 'editing', 'complete', 'refresh')),
  campaign_id uuid references public.campaigns(id) on delete set null,
  notes text not null default '',
  updated_at timestamptz not null default now(),
...
create table public.brand_references (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  kind text not null default 'file' check (kind in ('website', 'social', 'youtube', 'logo', 'guide', 'moodboard', 'photo', 'product', 'font', 'file')),
  label text not null,
  source_url text,
  storage_path text,
  metadata jsonb not null default '{}'::jsonb,
...
create index service_requests_workspace_type_created_idx on public.service_requests(workspace_id, request_type, created_at desc);
...
create index content_ideas_workspace_created_idx on public.content_ideas(workspace_id, created_at desc);
create index content_ideas_created_by_idx on public.content_ideas(created_by);
create index assistant_messages_workspace_created_idx on public.assistant_messages(workspace_id, created_at desc);
create index assistant_messages_user_idx on public.assistant_messages(user_id);
create index workspace_video_items_workspace_status_idx on public.workspace_video_items(workspace_id, status);
create index workspace_video_items_campaign_idx on public.workspace_video_items(campaign_id);
create index brand_references_workspace_created_idx on public.brand_references(workspace_id, created_at desc);
...
begin
  foreach tname in array array[
    'workspaces', 'workspace_members', 'workspace_subscriptions',
    ...
    'content_ideas', 'assistant_messages', 'workspace_video_items', 'brand_references'
  ] loop
...
grant all on public.workspace_video_items to service_role;
